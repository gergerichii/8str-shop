class Configure
    def Configure.configure(config, settings)
        # Set The VM Provider
        ...
        # Configure The Box
        config.vm.define settings["name"] ||= "yii2"
        config.vm.box = settings["box"] ||= "laravel/homestead"
        config.vm.box_version = settings["version"] ||= "> 0.0.0"
        config.vm.hostname = settings["hostname"] ||= "yii2"
        ...
        if settings.has_key?("networks")
            settings["networks"].each do |network|
                config.vm.network network["type"], ip: network["ip"] ||= nil, bridge: network["bridge"] ||= nil, netmask: network["netmask"] ||= "255.255.255.0"
            end
        end

        # Configure A Few VirtualBox Settings
        config.vm.provider "virtualbox" do |vb|
            vb.name = settings["name"] ||= "yii2"
...
#            ss.name = "Unmount all binded folders"
#            ss.inline = "cd /home/vagrant; find . -maxdepth 5 ! -path '*/vendor*' ! -path '*/.*' ! -path '*/node*' -type d -exec umount {} \\;"
#        end
...
#                    config.vm.provision "shell" do |ss|
#                        ss.name = "Checking for web folder " + site["to"]
#                        ss.path = scriptDir + "/check-folder.sh"
#                        ss.args = [site["to"]]
#                        ss.privileged = false
#                    end
                    ...
                    s.args = [site["map"], site["to"], site["port"] ||= "80", site["ssl"] ||= "443", site["php"] ||= "7.1", params ||= "", settings["commonFolder"] ||= '']
                end
		...
		# Конифигурация sendmail
        config.vm.provision "shell" do |s|
            s.name = "Clear sendmail"
            s.path = scriptDir + "/clear-sendmail.sh"
        end

		if settings.has_key?("ssmtp")
			config.vm.provision "shell" do |s|
				s.name = "Configure sendmail"
				s.path = scriptDir + "/install-sendmail.sh"
				s.args = [
					settings["ssmtp"]["root"],
					settings["ssmtp"]["mailhub"],
					settings["ssmtp"]["AuthUser"],
					settings["ssmtp"]["AuthPass"],
					settings["ssmtp"]["FromLineOverride"] ||= "YES",
					settings["ssmtp"]["UseSTARTTLS"] ||= "YES"
				]
			end			
		end
...
#                end
                ...
                config.vm.provision "shell" do |s|
                    s.inline = "echo \"\n# Set Virtual machine Environment Variable\nexport $1=$2\" >> /home/vagrant/.profile"
                    s.args = [var["key"], var["value"]]
                end
            end
