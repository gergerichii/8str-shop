...
use common\modules\cart\events\CartGroupModels;
use yii;
...
class CartService extends Component
{
    ...
    /** @var \common\modules\cart\interfaces\Cart|null  */
    private $cart = null;
     ...
     * Cart constructor.
     *
     * @param \common\modules\cart\interfaces\Cart    $Cart
     * @param \common\modules\cart\interfaces\Element $Element
     * @param array                                   $config
     */
    ...
    public function put(interfaces\CartElement $model, $count = 1, $options = [], $comment = null)
    {
        if (!$elementModel = $this->cart->getElement($model, $options)) {
            ...
            if(!$elementEvent->stop) {
                try {
                    ...
                    throw new Exception(current($e->getMessage()));
                }
            }
        } else {
    ...
    public function putWithPrice(interfaces\CartElement $model, $price = 0, $count = 1, $options = [], $comment = null)
    {
        if (!$elementModel = $this->cart->getElement($model, $options)) {
    ...
    /**
     * @return \common\modules\cart\interfaces\Cart|null
     */
